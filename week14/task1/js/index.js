...
const fMakerList = document.querySelector(".fmaker-list");
let list = "";
filmMaker.forEach((item) => {
  list += `<li>
<p class="fname">${item.name}</p>
<p class="fcareer">${item.career}</p>
  <a href="${item.films}">Фильмография</a>
  </li>`;
});
fMakerList.innerHTML = list;

document.getElementById("topList").innerHTML = filmMaker.map(getFilmList);
function getFilmList(item) {
